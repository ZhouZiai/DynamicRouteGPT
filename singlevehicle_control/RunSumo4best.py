...
def run():
    ...
    #         color_green=(0,255,0)
    #         traci.vehicle.setColor(v_id,color_green)
    v_id = "v1"
    if shortest_path is not None:
        route_id = "shortest_route"
        traci.route.add(route_id, shortest_edge_list)
    ...
    #以上代码为车辆添加行驶路线
        if v_id in traci.vehicle.getIDList():
            traci.gui.setZoom('View #0', 1000)
                #设置视角跟随车辆v1
            traci.gui.trackVehicle('View #0', "v1",depart=10, departPos="free", departSpeed=10)
            #     # 获取车辆所处车道
            ...
            # total_CO2 +=CO2
            # print(f"当前车辆CO2的总排放量：{total_CO2}")
            # fuel = traci.vehicle.getFuelConsumption("v1")
            # total_fuel +=fuel
            # print(f"当前车辆总油耗：{total_fuel}")
