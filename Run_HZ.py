...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        edge_list = [f"edge_{i}" for i in range(249)]
        ...
        for vechicle_id in vehicle_ids:
            if vechicle_id in traci.vehicle.getIDList():
                ...
                parts = lane_id.split("_")  # 先进行分割
                if len(parts) > 1:  # 确保有下划线可以分割
                    # 保留所有部分，除了最后一个
                    desired_parts = parts[:-1]
                    result = "_".join(desired_parts)  # 重新组合
                else:
                    result = lane_id  
                edge_list[i]= result
                edge_ = edge_list[i]
                    ...
                    if edge_ in pass_edges:
                        ...
                        #     pass_node.append(nod_id)
                            ...
                            try:
                                ...
                                print(f"'{chose_path[-1]}' 未在 short_edge_list 中找到，请检查数据或逻辑。short_edge_list is {shortest_edge_list }")
                            # 更新车辆路线
                            best_route= shortest_edge_list[:first_element_index]+chose_path+shortest_edge_list[last_element_index+1:]
...
if __name__ == '__main__':
    ...
    net_file = r'D:\test_dataset\hangzhou\hangzhou.net.xml'  # 路网文件
    rou_file = r'D:\test_dataset\hangzhou\hangzhou.rou.xml'  # 路网文件
    cfg_file = r'D:\test_dataset\hangzhou\hangzhou.sumocfg'
...
# network          start_edge      end_edge       num    1     5%     10%     15%   20%    25%    30%
# ingolstad21     -30482615#4      -315358244     4283         214     428  642     856